define([
  ...
  declare, array, event, lang, dom, domStyle, domClass, domConstruct, domGeometry, domQuery, on, keys, string
) {
    var kidney = declare(null, {
      ...
      findMatches: function(term, max) {
        ...
        for ( var i = 0; i < this.classList.length; i++ ) {
          ...
          if (  pos > -1 ) {
            // only show the class name, no need for the namespace stuff
            var classPieces = this.classList[i].split(".");
            var className = classPieces[classPieces.length - 1];
      ...
      makeClassLink: function(fullName, parent) {
        ...
        var classPieces = fullName.split(".");
        var className = classPieces[classPieces.length - 1].toLowerCase();
        // console.log("make link...class name: ", className);
        var href = this.baseUrl + className + ".html";
