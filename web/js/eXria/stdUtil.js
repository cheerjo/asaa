...
UtilGrid = {
  ...
  setCanEditByRow : function(psGrxId, poRow, paCol, pbEditable) {
    if (! !poRow) {
      ...
      for (var j = 0; j < vaColId.length; j++) {
        vsColId = vaColId [j];
        if (vcCtl.setCanEdit) vcCtl.setCanEdit(poRow, vsColId, false);
        else if (poRow.setAttribute) poRow.setAttribute(vsColId + "CanEdit", pbEditable);
        voBodyCol = vcCtl.getBodyColumn(null, vsColId);
...
UtilDataSet = {
   ...
   * @param {Array} paDataSetOrder 엑셀로 익스포트될 컬럼 오더 설정
   * @type Boolean
   * @return Export 성공 유무
   */  
  exportExcel : function(poDst, psFileName, pbHeader, pnStartIdx, pnEndIdx,
  paExcluColIds, poColIdLabel, paDataSetOrder) {
    ...
    try {
      ...
      if(!!paDataSetOrder && paDataSetOrder.length > 0){
        var vaTemp = paDataSetOrder;
        var vsTmpStr = ''; 
        vnLength = vaTemp.length;
        for (var k = 0; k < vnLength; k++) { 
          vsTmpStr = vsTmpStr + vaTemp[k] + ','
          
        }
        
        vsColIds = vsTmpStr.substring(0, vsTmpStr.length - 1);      
      }      
...
/*
 * < 변경내역 >
 * 4077줄: if문 추가함. [if (vcCtl.setCanEdit) vcCtl.setCanEdit(poRow, vsColId, false);]
 */
