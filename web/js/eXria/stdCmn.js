...
//      }
  ...
  restoreSelected : function(psGrxId, psColId, paIdx) {
    ...
    for (var i = vnSize - 1; i >= 0; i--) {
      ...
      if (psColId) {
        ...
        var vsOldStatus = voDataSet.getRowAttr(vnRowIndex, "old___Status");
        if (vsUptStatus == "U" || vsOldStatus == "U" || vsUptStatus == "D") {
  ...
  excelToDataSet : function(psDataSetId, psFileName, pbHeader, pnMinRow, pnMaxRow) {
    ...
    , "");
    var comm = conn.getCommand("commRead");
    if (!!pnMinRow) comm.setMinRow(pnMinRow);
    if (!!pnMaxRow) comm.setMaxRow(pnMaxRow);
  ...
  /**
   * Excel 파일 데이타를 사용자가 정의한 컬럼 순서에 따라 DataSet에 Export  
   * @param {JSON} poJson Excel 파일 데이타를 DataSet을 통해 읽어오기 위한 설정정보
   *    {                             <br />
   *      psDataSetId    - String/필수<br />
   *      psInstanceId   - Strubg/필수<br />
   *      psFileName     - String/필수<br /> 
   *      poInsCmnOrder  - Array/옵션 <br />
   *      pnStartRowNum  - Number/옵션<br />
   *      pnEndRowNum    - Number/옵션<br />
   *    }                             <br />
   * ex)
   *    {                                  <br />
   *      'psDataSetId'    : 'dst',        <br />
   *      'psInstanceId'   : 'ins',        <br />
   *      'psFileName'     : 'c:\text.xls',<br /> 
   *      'poInsCmnOrder'  : {'A':'aa'},   <br />
   *      'pnStartRowNum'  : 1,            <br />
   *      'pnEndRowNum'    : 20            <br />
   *    }                                  <br />    
   */
  excelToDataSetOrder : function(poJson) {
    var voErrMsg = {};
    try {
      if (!poJson && !poJson.psDataSetId && !poJson.psFileName && !poJson.psInstanceId) {
        voErrMsg.name = "Common.excelToDataSetOrder";
        voErrMsg.message = "psDataSetId, psInstanceId, psFileName은 반드시 값이 있어야 합니다. ";
        throw voErrMsg;
      }
      
      var voCmnOrder = poJson.poInsCmnOrder;       
      if(!!voCmnOrder){
        var vnTmp = 0;
        for(k in voCmnOrder){
          vnTmp = 1;
        ...
        if(0 === vnTmp){
          voErrMsg.name = "Common.excelToDataSetOrder";
          voErrMsg.message = "poInsCmnOrder는 반드시 attribute가 있어야 합니다. ex{'a' : 'x'} ";
          throw voErrMsg;          
        }
      }

      this.excelToDataSet(poJson.psDataSetId, poJson.psFileName, true, 
                          poJson.pnStartRowNum, poJson.pnEndRowNum);
                        
      if(voCmnOrder){
        var voIns = page.getInstance(poJson.psInstanceId);
        var voDst = page.getDataSet(poJson.psDataSetId);
        var vsTmp = voIns.getXML("/");
        var vnDstCnt = voDst.getColCnt();
        var vsColNm;
        var voMapper = {};
        var i=1;

        for(i=1; i <= vnDstCnt; i++){
          vsColNm = voDst.getColName(i);
          voMapper[vsColNm] = voDst.get(1, vsColNm);          
        }
        
        voDst.deleteRow(1);
        vsTmp = voIns.getXML("/");

        var voTargetMapper = voCmnOrder;                                                             
        for(p in voTargetMapper){
          vsTVal = voTargetMapper[p];
          vsOVal = voMapper[p];
          
          if(vsTVal !== vsOVal){
           for(t in voMapper){
             if(voMapper[t] === vsTVal){          
               vsTmp = vsTmp.simpleReplace('<'+ t +'>', '<'+ p + '_prex' +'>');
               vsTmp = vsTmp.simpleReplace('</'+ t +'>', '</'+ p + '_prex' +'>');
             }
           }  
          }     
        ...
        vsTmp = vsTmp.simpleReplace('_prex','');    
        voIns.pluginInstance.loadTXT(vsTmp, 0);      
      }
    }catch (err) {
      alert("Common"+ "\n" +
            "method : " + err.name + "\n" +
...
//      alert("로딩할 파일을 선택해주세요.");
  ...
  /**
   * 윈도우에 리사이즈 이벤트 발생시 SubPage, GridEx 컨트롤의 사이즈를 자동조정  
   * @param {JSON} paHList 넓이 사이즈가 변경될 SubPage, GridEx 
   * @param {JSON} paVList 높이 사이즈가 변경될 SubPage, GridEx
   * @param {JSON} paHVList 넓이와 높이 사이즈가 변경될 SubPage, GridEx 컨트롤의 배열
   * ex){                                                       <br />
   *      "GridIds" : [{id: 'grx1',width : 700, height : 150}], <br />
   *      "SubPageIds" : []                                     <br />
   *    }                                                       <br />    
   */
  resizeWithWindow : function(paHList, paVList, paHVList){
    var voErrMsg = {};
    ...
    try {
      if (!paHList && !paVList && !paHVList) {
        voErrMsg.name = "Common.resizeWithWindow";
        voErrMsg.message = "paHList, paVList, paHVList 중 한가지는 반드시 값이 있어야 합니다. ";
        throw voErrMsg;
      }

      {
        ...
        for(i=0; i<vnLength; i++){
          if(!!arguments[i] && !arguments[i]['GridIds'] && !arguments[i]['SubPageIds']){
            voErrMsg.name = "Common.resizeWithWindow";
            voErrMsg.message = "아규먼트의 속성중에 GridIds 혹은 SubPageIds 둘중하는 반드시 값이 있어야 합니다.  ";
            throw voErrMsg;            
          }
        }
        ...
        for(i=0; i<vnLength; i++){
          if( !!arguments[i] &&
              0 === arguments[i]['GridIds'].length && 
              0 === arguments[i]['SubPageIds'].length ){
            voErrMsg.name = "Common.resizeWithWindow"
            voErrMsg.message = "아규먼트의 속성인 GridIds 혹은 SubPageIds의 배열의 길이는 0보다 커야 합니다. ";
            throw voErrMsg;            
          }
        }                
      }
      ...
      var voFunc = function() {
        ...
        var voResizeFunc = function(paList, psCtrlType, psResizeType){
         var vcCtl = null;
         var vnSize = paList.length;
         ...
         for(var i = 0; i < vnSize; i++) {
           ...
           switch(psResizeType){
              case 'H' :
                ...
                vcCtl.width = (vcCtl.width < paList[i].width) ? paList[i].width : vcCtl.width;
                break;                
              
              case 'V' :                     
                vcCtl.height = vcCtl.height + vnVGap;
                vcCtl.height = (vcCtl.height < paList[i].height) ? paList[i].height : vcCtl.height;                
                break;                
           ...
           if('GridIds' === psCtrlType) {                                                                
              if(vcCtl.width > 30 && vcCtl.height > 60){
               vcCtl.refresh(null, null, true);
              }         
           } 
           
           if('SubPageIds' === psCtrlType){
              //var vnWidth = vcCtl.width + vnHGap;
              //var vnHeight = vcCtl.height + vnVGap;             
              //vnWidth = (vnWidth < paList[i].width) ? paList[i].width : vnWidth;
              //vnHeight = (vnHeight < paList[i].height) ? paList[i].height : vnHeight;             
              //vcCtl.applyAttr("width", vnWidth);
              //vcCtl.applyAttr("height", vnHeight);
              vcCtl.applyAttr("width", vcCtl.width);
              vcCtl.applyAttr("height", vcCtl.height);              
           }
         }          
        };        
        
        if(!!paHList && !!(paHList['GridIds'])) voResizeFunc(paHList['GridIds'], 'GridIds', 'H');
        
        if(!!paHList && !!(paHList['SubPageIds'])) voResizeFunc(paHList['SubPageIds'], 'SubPageIds', 'H');  
                        
        if(!!paVList && !!(paVList['GridIds'])) voResizeFunc(paVList['GridIds'], 'GridIds', 'V');
        
        if(!!paVList && !!(paVList['SubPageIds'])) voResizeFunc(paVList['SubPageIds'], 'SubPageIds', 'V');  
        
        if(!!paHVList && !!(paHVList['GridIds'])) voResizeFunc(paHVList['GridIds'], 'GridIds', 'HV');
        
        if(!!paHVList && !!(paHVList['SubPageIds'])) voResizeFunc(paHVList['SubPageIds'], 'SubPageIds', 'HV');  
  ...
  /**
   * 윈도우에 리사이즈 이벤트 발생시 SubPage, GridEx 컨트롤의 사이즈를 자동조정  
   * @param {Array} paHList 넓이 사이즈가 변경될 SubPage, GridEx 컨트롤의 배열 
   * @param {Array} paVList 높이 사이즈가 변경될 SubPage, GridEx 컨트롤의 배열
   * @param {Array} paHVList 넓이와 높이 사이즈가 변경될 SubPage, GridEx 컨트롤의 배열
   * @param {JSON}  poHVMNSize 모든 SubPage 혹은 GridEx의 사이즈 변경
   * ex){                        <br />
   *      "width" : 800,         <br />
   *      "height" : 600         <br />
   *    }                        <br />    
   */  
  resizeWindow : function(paHList, paVList, paHVList, poHVMNSize) {
   page.mnWidth = page.window.document.documentElement.clientWidth;
   page.mnHeight = page.window.document.documentElement.clientHeight;
   var vsEventCnt = "resizeEventCnt_" + page.id;    
   ...
   var vbSubPageUse = false;
   var vbGridExUse = false;
   
   if(!!poHVMNSize){
     vbSubPageUse = poHVMNSize.SubPageUse || false;
     vbGridExUse = poHVMNSize.GridExUse || false; 
   }
   ...
   var voFunc = function() {
     ...
     for(var i = 0; i < vnSize; i++) {
       ...
       if(voApplyAttrMap[vcCtl.id] == null) {         
         ...
         if(vbGridExUse){
          vcCtl.width = (vcCtl.width < poHVMNSize.GridEx.width) ? poHVMNSize.GridEx.width : vcCtl.width;                     
          vcCtl.height = (vcCtl.height < poHVMNSize.GridEx.height) ? poHVMNSize.GridEx.height : vcCtl.height;
         }
         if(vcCtl.width > 30 && vcCtl.height > 60){
          vcCtl.refresh(null, null, true);  
         }         
         ...
         if(vbSubPageUse){
           vnWidth = (vnWidth < poHVMNSize.SubPage.width) ? poHVMNSize.SubPage.width : vnWidth;
           vnHeight = (vnHeight < poHVMNSize.SubPage.height) ? poHVMNSize.SubPage.height : vnHeight;
         }
...
//       //MinSize 입력값과 비교후 작으면 onresize 미실행 
//       if(!!poMinSize){
//          if(poMinSize.width >= page.window.document.documentElement.clientWidth
//           ||poMinSize.height >= page.window.document.documentElement.clientHeight){
//             return false;     
//          }
//       }
