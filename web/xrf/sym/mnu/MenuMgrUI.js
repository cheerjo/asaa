var MenuMgrUI = function() {
this.rendering = function(poPage) {
var voCtl = null;
poPage.canvas.backgroundColor = null;
poPage.canvas.backgroundRepeat = null;
poPage.canvas.backgroundPosition = null;
poPage.canvas.focusBorderColor = null;
poPage.canvas.focusBorderStyle = null;
poPage.canvas.contextMenuId = null;
poPage.canvas.horizontalAlign = null;
poPage.canvas.width = 1620;
poPage.canvas.height = 735;
poPage.canvas.refresh();
voCtl = poPage.canvas.createControl("label","lblSubTitle01",10,440,1601,36);
voCtl.className = "lbl_subtitle";
voCtl.outerClassName = "lbl_subtitle_outer";
voCtl.tabIndex = 0;
voCtl.zIndex = 0;
voCtl.value = "기본정보";
voCtl.borderWidth = 0;
voCtl.textAlign = "left";
poPage.canvas.appendControl(voCtl);
voCtl = poPage.canvas.createControl("group","grp1",10,479,1601,169);
voCtl.tabIndex = 0;
voCtl.zIndex = 1;
voCtl.overflow = "visible";
voCtl.borderColor = "#A5BACC";
voCtl.borderWidth = 0;
voCtl.borderColor = "#A5BACC";
voCtl.borderLeftWidth = 1;
voCtl.borderRightWidth = 1;
voCtl.borderBottomWidth = 1;
{
var voCtl_InnerCtl = null;
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine1",0,0,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 0;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl65",0,1,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 1;
voCtl_InnerCtl.value = "메뉴코드";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbMenuCd",120,5,101,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/menuCd");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.zIndex = 2;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl58",280,1,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 3;
voCtl_InnerCtl.value = "상위메뉴코드";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine2",0,28,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 4;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl59",0,29,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 5;
voCtl_InnerCtl.value = "메 뉴 명";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine3",0,56,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 6;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl72",0,57,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 7;
voCtl_InnerCtl.value = "linkUrl";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine4",0,112,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 8;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl78",0,113,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 9;
voCtl_InnerCtl.value = "비      고";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("radio","rdbUseYn",400,145,150,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/useYn");
voCtl_InnerCtl.className = "rdb_form";
voCtl_InnerCtl.outerClassName = "rdb_form_outer";
voCtl_InnerCtl.zIndex = 10;
voCtl_InnerCtl.displayMode = "horizontal";
voCtl_InnerCtl.fontSize = 9;
voCtl_InnerCtl.itemgroup.width = 75;
var vcItem = voCtl_InnerCtl.addItemFront("사용", "Y");
vcItem.selected = true;
var vcItem = voCtl_InnerCtl.addItemFront("미사용", "N");
voCtl_InnerCtl.borderWidth = 0;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine5",0,140,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 11;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl84",0,141,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 12;
voCtl_InnerCtl.value = "표시순서";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl86",280,141,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 13;
voCtl_InnerCtl.value = "사용여부";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine6",0,168,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 14;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbUpMenuCd",400,5,100,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/upMenuCd");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 15;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbMenuNm",119,33,472,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/menuNm");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 16;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbLinkUrl",119,61,1472,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/linkUrl");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 17;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbReMark",119,117,1472,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/reMark");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 18;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbDplyNo",120,145,101,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/dplyNo");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 19;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblLine3_copy_1",0,84,1611,1);
voCtl_InnerCtl.outerClassName = "lbl_formvalbox_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 20;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lbl87",0,85,110,28);
voCtl_InnerCtl.className = "lbl_formattr";
voCtl_InnerCtl.outerClassName = "lbl_formattr_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 21;
voCtl_InnerCtl.value = "메뉴설명";
voCtl_InnerCtl.borderStyle = "solid";
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbMenuDesc",119,89,1472,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/reqSymMenuVO/menuDesc");
voCtl_InnerCtl.className = "ipt_form";
voCtl_InnerCtl.outerClassName = "ipt_form_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 22;
voCtl_InnerCtl.readOnly = false;
voCtl.addChild(voCtl_InnerCtl);
}
poPage.canvas.appendControl(voCtl);
voCtl = poPage.canvas.createControl("group","grp3",10,10,1601,421);
voCtl.tabIndex = 0;
voCtl.zIndex = 2;
{
var voCtl_InnerCtl = null;
voCtl_InnerCtl = poPage.canvas.createControl("label","lblSerchArea1",10,10,1581,35);
voCtl_InnerCtl.outerClassName = "lbl_srcharea_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 0;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("label","lblOrgnClsfCd_copy_1",20,18,60,20);
voCtl_InnerCtl.className = "lbl_srch";
voCtl_InnerCtl.outerClassName = "lbl_srch_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 1;
voCtl_InnerCtl.value = "메뉴명";
voCtl_InnerCtl.borderWidth = 0;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("input","ipbSchMenuNm",90,18,261,20);
voCtl_InnerCtl.data.setRef("requestData", "/root/searchVO/searchKeyword");
voCtl_InnerCtl.className = "ipt_srch";
voCtl_InnerCtl.zIndex = 2;
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("button","btnSrch",360,15,77,24);
voCtl_InnerCtl.className = "btn_srch";
voCtl_InnerCtl.outerClassName = "btn_srch_outer";
voCtl_InnerCtl.tabIndex = 0;
voCtl_InnerCtl.zIndex = 3;
voCtl_InnerCtl.value = "조회";
voCtl_InnerCtl.onclick = function(e) { page.onClick_btnSrch(e); };
voCtl.addChild(voCtl_InnerCtl);
voCtl_InnerCtl = poPage.canvas.createControl("gridex","grxSymMenu",10,45,1581,366);
voCtl_InnerCtl.zIndex = 4;
voCtl_InnerCtl.datasetId = "dstResSymMenuList";
voCtl_InnerCtl.cfg.editing = 2;
voCtl_InnerCtl.cfg.sorted = false;
voCtl_InnerCtl.cfg.sorting = false;
voCtl_InnerCtl.cfg.idColumn = "ghc1";
voCtl_InnerCtl.cfg.multiSelect = true;
{
var voCtl_InnerCtl_header = voCtl_InnerCtl.createHeader();
{
var voCtl_InnerCtl_header_cols = voCtl_InnerCtl_header.createCols();
var voCtl_InnerCtl_header_cols_colsRow;
voCtl_InnerCtl_header_cols_colsRow = voCtl_InnerCtl_header_cols.createColsRow();
var voCtl_InnerCtl_header_cols_colsRow_headCol;
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghc1", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "No";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcMenuCd", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "메뉴코드";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcUpMenuCd", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "상위메뉴코드";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcMenuNm", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "메뉴명";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcMenuDesc", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "linkUrl";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghc6", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "메뉴설명";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcDplyNo", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "표시순서";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
voCtl_InnerCtl_header_cols_colsRow_headCol = voCtl_InnerCtl_header_cols_colsRow.createHeadColumn("ghcUseYn", null, null);
voCtl_InnerCtl_header_cols_colsRow_headCol.colspan = 1;
voCtl_InnerCtl_header_cols_colsRow_headCol.textAlign = "center";
voCtl_InnerCtl_header_cols_colsRow_headCol.className = "ghc";
voCtl_InnerCtl_header_cols_colsRow_headCol.value = "사용여부";
voCtl_InnerCtl_header_cols_colsRow.addColumn(voCtl_InnerCtl_header_cols_colsRow_headCol);
var voCtl_InnerCtl_header_cols_colsRow_functionCol;
voCtl_InnerCtl_header_cols.addColsRow(voCtl_InnerCtl_header_cols_colsRow);
voCtl_InnerCtl_header.addCols(voCtl_InnerCtl_header_cols, "center");
}
voCtl_InnerCtl.addHeader(voCtl_InnerCtl_header);
}
{
var voCtl_InnerCtl_body = voCtl_InnerCtl.createBody();
{
var voCtl_InnerCtl_body_cols = voCtl_InnerCtl_body.createCols();
var voCtl_InnerCtl_body_cols_colsRow;
voCtl_InnerCtl_body_cols_colsRow = voCtl_InnerCtl_body_cols.createColsRow();
voCtl_InnerCtl_body_cols_colsRow.height = 20;
var voCtl_InnerCtl_body_cols_colsRow_bodyCol;
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcNo", "ghc1", "Text", null, null);
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 50;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.textAlign = "center";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcMenuCd", "ghcMenuCd", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 98;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "menuCd";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcUpMenuCd", "ghcUpMenuCd", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 90;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "upMenuCd";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcMenuNm", "ghcMenuNm", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 214;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "menuNm";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcLinkUrl", "ghcMenuDesc", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 396;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "linkUrl";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcMenuDesc", "ghc6", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 551;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "menuDesc";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcDplyNo", "ghcDplyNo", "Text", null, "");
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 84;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "dplyNo";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols_colsRow_bodyCol = voCtl_InnerCtl_body_cols_colsRow.createBodyColumn("gbcUseYn", "ghcUseYn", "Text", null, null);
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regMask = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.regFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.groupFormat = null;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.width = 78;
voCtl_InnerCtl_body_cols_colsRow_bodyCol.datasetCol = "useYn";
voCtl_InnerCtl_body_cols_colsRow.addColumn(voCtl_InnerCtl_body_cols_colsRow_bodyCol);
voCtl_InnerCtl_body_cols.addColsRow(voCtl_InnerCtl_body_cols_colsRow);
voCtl_InnerCtl_body.addCols(voCtl_InnerCtl_body_cols, "center");
}
voCtl_InnerCtl.addBody(voCtl_InnerCtl_body);
}
voCtl_InnerCtl.onGridexDblClick = function(e) { page.onGridexDblClick_grxSymMenu(e); };
voCtl.addChild(voCtl_InnerCtl);
}
poPage.canvas.appendControl(voCtl);
voCtl = poPage.canvas.createControl("button","btnInit",1413,448,54,20);
voCtl.className = "btn_inn02s";
voCtl.outerClassName = "btn_inn_outer";
voCtl.zIndex = 3;
voCtl.value = "초기화";
voCtl.onclick = function(e) { page.onClick_btnInit(e); };
poPage.canvas.appendControl(voCtl);
voCtl = poPage.canvas.createControl("button","btnSave",1480,448,54,20);
voCtl.className = "btn_inn02s";
voCtl.outerClassName = "btn_inn_outer";
voCtl.zIndex = 4;
voCtl.value = "저장";
voCtl.onclick = function(e) { page.onClick_btnSave(e); };
poPage.canvas.appendControl(voCtl);
voCtl = poPage.canvas.createControl("button","btnDelete",1545,448,54,20);
voCtl.className = "btn_inn02s";
voCtl.outerClassName = "btn_inn_outer";
voCtl.tabIndex = 0;
voCtl.zIndex = 5;
voCtl.value = "삭제";
voCtl.onclick = function(e) { page.onClick_btnDelete(e); };
poPage.canvas.appendControl(voCtl);

};

};