...
<%
/**
 * @Class Name : sptIptnRccListPopUp.jsp
 * @Description : 기록계정보 조회
...
<head>
    ...
    <script type="text/javascript">
       ...
       // 	form.pageIndex.value = "1";
        ...
        // 기록계정보 추가
        function fn_insert() {
        	var form = document.sptIptnRccForm;
        	var checkCnt = 0; // 체크카운트
        	var sncChk = form.checkDownload;
        	
        	if(document.forms["sptIptnRccForm"]["checkDownload"].length > 1){
        		
        		
        		for (var i=0; i<document.forms["sptIptnRccForm"]["checkDownload"].length; i++) {
        		 ...
        		 var lo = 0;
        		    if( checkCnt >= 1 ) {
        		        for (var i=0; i<document.forms["sptIptnRccForm"]["checkDownload"].length; i++) {
        		            if( form.checkDownload[i].checked ) {
        		                if(lo == 0){
        		                    document.forms["sptIptnRccForm"]["fileList"].value = document.forms["sptIptnRccForm"]["filename"][i].value;
        		                } else {
        		                    document.forms["sptIptnRccForm"]["fileList"].value = document.forms["sptIptnRccForm"]["fileList"].value + "|" 
        		                        + document.forms["sptIptnRccForm"]["filename"][i].value;
        		                }
        		                lo = lo + 1;
        		            }
        		        }
        	    ...
        	    var fileList = document.forms["sptIptnRccForm"]["fileList"].value;
        		
        	}else {
        		if(document.forms["sptIptnRccForm"]["checkDownload"].checked){
        			var fileList = document.forms["sptIptnRccForm"]["filename"].value;
        		}else{
        			alert("최소한 하나의 대상은 선택하여야 합니다.");
        			return;
        		}
        	...
        	/*
        	if(checkFiniCalandar - checkInitCalandar < 3600000){
        		alert("구간의 간격을 1시간 미만으로 지정할 수 없습니다.");
        		return;
        	}
        	*/
...
<form:form commandName="sptIptnRccVO" name="sptIptnRccForm" method="post">
	        ...
	        <div class="category_popup_wrapper category_popup_wrapper2">
		        ...
		        <div class="category_table zipcode">
		            <div class="dataTables_wrapper">
		                ...
		                <table class="display data_table2 dataTable">
		                    ...
		                    <tbody>
		                        <c:forEach var="result" items="${sptIptnRccSearchList}" varStatus="status">
		                            <tr>
		                                <td><input type="checkbox" title="선택" name="checkDownload"></td>
		                                <td class="zipcode"><c:out value="${result.rccMdl}"/></td>      <!-- 모델명 -->
		                                <td class="zipcode"><c:out value="${result.rccIstlPos}"/></td>  <!-- 설치위치 -->
			                            <td class="zipcode"><c:out value="${result.rccCmpy}"/></td>     <!-- 제작사 -->
			                            <td class="zipcode"><c:out value="${result.rccAuthIntt}"/></td> <!-- 인증기관 -->
		                            </tr>
		                        ...
		                        <c:if test="${fn:length(sptIptnRccSearchList) == 0}">
		                            <tr>
		                              <!--    <td colspan="2">조회된 기록계정보가 없습니다.</td> -->
		       ...
		       <!-- btn_group -->
                <div class="btn_group">
       				...
       				<div class="btn"><a href="#" class="css_button blue" onclick="fn_insert(document.sptIptnRccForm.listCnt.value)"><span>확인</span></a></div>
