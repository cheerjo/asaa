...
<script type="text/javascript">
 <!--
  ...
  function goLeftSearch(){ 
      document.leftFctyForm.action = "<c:url value='/scn/ScnRccManager.do'/>"; 
 ...
 function fcFctyMgrCenter() { 
     $.ajax({
         type : "POST" //"POST", "GET"
         , async : false //true, false
         , url : "${ctx}/scn/rptFctyMgrCenterInfo.do" //Request URL
         , dataType : "html" //전송받을 데이터의 타입
                                    //"xml", "html", "script", "json" 등 지정 가능
                                    //미지정시 자동 판단
         //, timeout : 30000 //제한시간 지정
 ...
 function fcFctyMgrRight() { 
     $.ajax({
         type : "POST" //"POST", "GET"
         , async : false //true, false
         , url : "${ctx}/scn/rptFctyMgrRightInfo.do" //Request URL
         , dataType : "html" //전송받을 데이터의 타입
                                    //"xml", "html", "script", "json" 등 지정 가능
                                    //미지정시 자동 판단
         //, timeout : 30000 //제한시간 지정
				...
				<div class="widget_header3" id="FctyLeftScl"  style="height:550px;overflow-y:scroll;">           
					<table class="buil_list_tb" id="chTable">
					  ...
					  <c:forEach var="rptmgrList" items="${rptMgrList}" varStatus="status"> 	
						<tr id="fctycd:${rptmgrList.iptDt}:${rptmgrList.fctyCd}:${rptmgrList.conDaGbcd}:${rptmgrList.gvmtCd}">
